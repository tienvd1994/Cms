...
namespace MyWeb.Core
{
    ...
    public partial class CommonHelper
    {
        ...
        public static string EnsureSubscriberEmailOrThrow(string email)
        {
            ...
            {
                throw new MyWebException("Email is not valid.");
            }
        ...
        public static void SetProperty(object instance, string propertyName, object value)
        {
            ...
            if (pi == null)
                throw new MyWebException("No property '{0}' found on the instance of type '{1}'.", propertyName, instanceType);
            if (!pi.CanWrite)
                throw new MyWebException("The property '{0}' on the instance of type '{1}' does not have a setter.", propertyName, instanceType);
        ...
        public static Image Resize(Image image)
        {
            ...
            else if (sourceWidth <= 480 && sourceHeight <= 360)
            {
                return image;
            }
            else
            {
                newWidth = Math.Round(sourceWidth / 1.25);
                newHeight = Math.Round(sourceHeight / 1.25);
            }

            Bitmap bmPhoto = new Bitmap((int)newWidth, (int)newHeight, PixelFormat.Format32bppPArgb);
            bmPhoto.SetResolution(image.HorizontalResolution, image.VerticalResolution);

            Graphics grPhoto = Graphics.FromImage(bmPhoto);
            grPhoto.InterpolationMode = InterpolationMode.HighQualityBicubic;

            grPhoto.DrawImage(image, new Rectangle(0, 0, (int)newWidth, (int)newHeight), new Rectangle(0, 0,
            (int)sourceWidth, (int)sourceHeight), GraphicsUnit.Pixel);
        ...
        public static string NameToSlug(string stringName)
        {
            string stringReturn = "";
            Regex regex = new Regex("\\p{IsCombiningDiacriticalMarks}+");
            stringReturn = Regex.Replace(stringName, "[^\\w\\s]", string.Empty).Replace(" ", "-").ToLower();
            string stringFormD = stringReturn.Normalize(System.Text.NormalizationForm.FormD);
            return regex.Replace(stringFormD, string.Empty).Replace("đ", "d");
        }

        public static string VietNamToPlainText(string sContent)
        {
            return Ucs2Convert(sContent);
        }
